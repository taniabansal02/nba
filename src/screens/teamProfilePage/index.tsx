  ...
  return (
    <View style={styles.mainView}>
      ...
      <View style={styles.profileView}>
        <View style={styles.imgView}>
          <View style={[styles.teamIcons, {backgroundColor: generateColor()}]}>
            <Text style={styles.text}>
              {' '}
              {item?.city[0] ? item.city[0] : null}
              {item?.name[0] ? item.name[0] : null}{' '}
            </Text>
          </View>
        </View>
        ...
        <ProfileCard
          title={Strings.teams.division}
          Flex={0.62}
          val={item.division}
        />
        <ProfileCard title={Strings.teams.city} Flex={0.57} val={item.city} />
        <ProfileCard
          title={Strings.teams.conference}
          Flex={0.69}
          val={item.conference}
        />
        <ProfileCard
          title={Strings.teams.teamAbb}
          Flex={0.66}
          val={item.abbreviation}
        />
      </View>
    </View>
  );
